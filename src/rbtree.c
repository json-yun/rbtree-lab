...
void __rotate_left(node_t *N, node_t *P) {
    P->right = N->left;
    if (N->left != NULL) {
        N->left->parent = P;
    }
...
void __rotate_right(node_t *N, node_t *P) {
    P->left = N->right;
    if (N->right != NULL) {
        N->right->parent = P;
    }
...
node_t *_rotate_tree(rbtree *t, node_t *N) {
    ...
    if (N == P->right) __rotate_left(N, P);
    else __rotate_right(N, P);
